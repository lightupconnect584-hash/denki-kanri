    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-4 sm:py-6">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="text" inputMode="numeric" value={form.price}
                        ...
                        className="w-full border border-gray-300 rounded-lg pl-6 pr-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        placeholder="0" />
                    </div>
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <div className="flex gap-2 mb-2">
                    ...
                    <button type="button" onClick={addPart}
                      className="text-xs bg-gray-100 text-gray-700 border border-gray-300 rounded-lg px-3 py-2 hover:bg-gray-200 transition">追加</button>
                  </div>
        ...
        {showCatManager && role === "ADMIN" && (
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 px-4">
            <div className="bg-white rounded-2xl w-full max-w-sm max-h-[80vh] overflow-y-auto p-5 shadow-xl">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-gray-800">大枠の管理</h3>
      ...
      {expanded && (
        <div className="mt-3 space-y-2 border-t border-gray-100 pt-3">
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
            {item.updatedOn && (
              <div><span className="text-gray-400">更新日</span><span className="ml-1 text-gray-700">{new Date(item.updatedOn).toLocaleDateString("ja-JP")}</span></div>
            )}
            {item.price != null && (
              <div><span className="text-gray-400">金額（税別）</span><span className="ml-1 text-gray-700">¥{item.price.toLocaleString()}</span></div>
            )}
            {item.replacementCost != null && (
              <div><span className="text-gray-400">交換金額（税別）</span><span className="ml-1 text-gray-700">¥{item.replacementCost.toLocaleString()}</span></div>
            )}
          </div>
